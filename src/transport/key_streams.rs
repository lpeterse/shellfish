...
pub struct KeyStreamX25519 {
    k: [u8; 32], // TODO: use SharedSecret
    h: SessionId,
    sid: SessionId,
}

impl KeyStreamX25519 {
    pub fn new(k: [u8; 32], h: SessionId, sid: SessionId) -> Self {
        Self { k, h, sid }
    }
    ...
    pub fn key_32_32(&self, idx: char, k1: &mut [u8; 32], k2: &mut [u8; 32]) {
        ...
        let _ = Encode::encode(&MPInt(&self.k), &mut sha2);
...
mod tests {
    ...
    fn test_key_streams_sha2_01() {
        ...
        let ks = KeyStreamX25519::new(k, SessionId::new(h.clone()), SessionId::new(h));

        let mut c1_ = [0; 32];
        let mut c2_ = [0; 32];
        ks.key_32_32('C', &mut c1_, &mut c2_);
        assert_eq!(c1_, c1, "c1");
        assert_eq!(c2_, c2, "c2");

        let mut d1_ = [0; 32];
        let mut d2_ = [0; 32];
        ks.key_32_32('D', &mut d1_, &mut d2_);
        assert_eq!(d1_, d1, "d1");
        assert_eq!(d2_, d2, "d2");
    }

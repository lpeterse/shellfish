...
impl <'a> KeyStreamSha256<'a> {
    ...
    fn new(state: &'a mut Sha256, k: &'a [u8], h: &'a [u8], sid: &'a [u8], idx: char) -> Self {
        ...
        Self { k, h, state, stream, position: 0 }
    }
    ...
    pub fn read(&mut self, buf: &mut [u8]) {
        ...
        while requested > available {
            ...
            self.stream.extend_from_slice(&self.state.result_reset()[..]);
...
mod test {
    ...
    #[test]
    fn test_key_streams_sha2() {
        let k = [107,228,126,33,91,152,255,218,241,220,23,167,79,146,12,100,222,142,141,72,246,81,24,199,127,89,24,29,124,166,187,14];
        let h = [143,162,77,88,20,122,164,90,216,15,8,149,23,47,66,157,242,12,176,63,153,120,103,133,17,36,10,69,6,145,250,211];
        let sid = h;
        let c1 = [35,83,168,202,23,231,195,6,115,123,255,191,43,255,229,67,98,137,190,144,108,174,108,161,250,15,170,67,142,10,102,230];
        let c2 = [208,1,211,152,131,216,216,233,134,111,193,40,199,147,160,146,106,253,28,52,89,128,0,225,61,213,79,108,116,63,4,20];
        let c3 = [94,43,166,97,133,172,152,2,25,11,160,232,103,139,27,114,221,159,190,65,59,46,138,76,237,254,51,56,37,43,86,130];
        let d1 = [184,79,161,100,101,46,182,213,100,94,243,107,150,176,38,24,244,253,153,109,83,174,5,231,139,201,30,78,88,167,227,41];
        let d2 = [172,158,149,34,60,215,124,232,242,63,133,44,219,188,109,5,24,230,203,243,189,84,85,5,162,99,163,190,201,197,78,27];
        let d3 = [22,59,101,191,103,154,108,11,161,26,111,59,28,53,216,56,218,138,28,112,145,205,14,56,169,159,134,220,119,201,5,244];

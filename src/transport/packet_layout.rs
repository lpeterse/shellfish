...
impl PacketLayout {
    ...
    pub fn new(payload_len: usize, block_len: usize, mac_len: usize) -> PacketLayout {
        let padding_len = {
            ...
            if p < Self::PADDING_MIN_SIZE { p += block_len };
            while p + l < Self::PACKET_MIN_SIZE { p += block_len };
            p
        };
        PacketLayout {
            padding_len,
    ...
    pub fn buffer_len(&self) -> usize {
        Self::PACKET_LEN_SIZE + Self::PADDING_LEN_SIZE + self.payload_len + self.padding_len + self.mac_len
    }
    ...
    pub fn padding_range(&self) -> Range<usize> {
        Range {
            ...
            end: Self::PACKET_LEN_SIZE + Self::PADDING_LEN_SIZE + self.payload_len + self.padding_len,
        }
    }
    ...
    pub fn mac_range(&self) -> Range<usize> {
        Range {
            start: Self::PACKET_LEN_SIZE + Self::PADDING_LEN_SIZE + self.payload_len + self.padding_len,
            end: Self::PACKET_LEN_SIZE + Self::PADDING_LEN_SIZE + self.payload_len + self.padding_len + self.mac_len,
        }
    }
    pub fn pad_zero(&self, buf: &mut [u8]) {
        buf[Self::PACKET_LEN_SIZE] = self.padding_len as u8;
        for i in &mut buf[self.padding_range()] { *i = 0 };
    }

...
mod test {
    ...
    fn test_encrypt_decrypt_eq() {
        ...
        // Decrypt
        assert_eq!(Some(&payload[..]), ctx.decrypt_packet(pc, &mut buf).map(|x| &x[1..][..payload.len()]));
    }
    ...
    #[test]
    fn test_poly1305() {
        let key = [2,36,186,199,156,219,160,59,58,72,185,13,36,91,46,55,10,206,108,143,250,250,227,41,164,26,13,4,248,136,67,35];
        let msg = [1,2,3,4,5,6,7,8];
        let tag = [5,144,82,159,246,206,249,18,184,150,179,37,193,39,161,138];

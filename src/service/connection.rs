...
impl TryFrom<Response> for () {
    type Error = ();
    fn try_from(x: Response) -> Result<Self, ()> {
        match x {
            Response::Ok => Ok(()),
            _ => Err(()),
        }
...
impl TryFrom<Response> for Session {
    ...
    fn try_from(x: Response) -> Result<Self, ()> {
        match x {
            Response::OpenSession(c) => Ok(c),
            _ => Err(()),
        }
    }
}

impl TryFrom<Response> for Result<Session, ChannelOpenFailureReason> {
    type Error = ();
    fn try_from(x: Response) -> Result<Self, ()> {
        match x {
            Response::OpenFailure(reason) => Ok(Err(reason)),
...
impl From<ChannelOpenFailureReason> for Response {
    fn from(x: ChannelOpenFailureReason) -> Self {
        Response::OpenFailure(x)
    }
}

impl TryFrom<Request> for ChannelOpenRequest {
    type Error = ();
    fn try_from(x: Request) -> Result<ChannelOpenRequest, ()> {
        match x {
            Request::ChannelOpen(x) => Ok(x),
            _ => Err(()),
        }
    }
}
